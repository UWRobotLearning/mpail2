...
class MPAILPlanner(torch.nn.Module):
    ...
    def step(self, obs, use_prev_opt:bool=True, deterministic:bool=False) -> torch.Tensor:
        ...
        plan = self.optimize(
            obs, iter=self.cfg.opt_iters - 1, deterministic=deterministic or not self.cfg.stoch_act
    ...
    def optimize(self, obs, iter=None, deterministic:bool=False) -> torch.Tensor:
        ...
        if not deterministic:
            self._opt_controls[:] = self._sample_plan(sample_from_gaussian=self.cfg.gaussian_act)
        else:
            self._opt_controls[:] = torch.sum(self._prior_controls * self._weights[..., None, None], dim=-3)

        return self._opt_controls

    def _sample_plan(self, sample_from_gaussian: bool = False) -> torch.Tensor:
        '''
        ...
        # Original: multinomial sampling from weighted rollouts
        K, T, nu = self._prior_controls.shape[-3:]
        indices = torch.multinomial(self._weights, num_samples=1, replacement=True)  # [..., 1]
        batch_shape = indices.shape[:-1]
        gather_inds = indices.unsqueeze(-1).unsqueeze(-1).expand(*batch_shape, 1, T, nu)  # [..., 1, T, nu]
        sampled_plans = self._prior_controls.gather(dim=-3, index=gather_inds)  # [..., 1, T, nu]

        return sampled_plans.squeeze(-3)  # [..., T, nu]
